...
function renderFeaturedCitiesGrid() {
  ...
  // Limit homepage to first 3 cities
  allCitiesData.slice(0, 3).forEach(city => {
    const card = document.createElement('div');
...
function renderItineraryTimeline() {
  ...
  const itineraries = customItineraries[activeCityData.id]?.[appLanguage] || customItineraries[activeCityData.id]?.['en'] || [];
